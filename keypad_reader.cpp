...
struct signal_state_t {
  ...
  signal_state_t(unsigned int integrator) : debouncers(NUM_LINES, integrator) {}
};
...
int get_lines(unsigned int a_pin, unsigned int b_pin, unsigned int d_pin,
              unsigned int up_pin, unsigned int down_pin, const char *bias,
              const char *active_low, struct gpiod_line_bulk *lines) {
...
void sample_tick() {
  ...
  for (uint i = 0; i < NUM_LINES; ++i) {
    values[i] =
        static_cast<int>(signal_state->debouncers[i].add_value(values[i]));
  }

  const uint8_t cur_vol_val = (values[UP_OFF] << 0) | (values[DOWN_OFF] << 1);
...
void handle_sigalrm(int sig) { read_io = 1; }

int main(int argc, char *argv[]) {
  if (argc < 7) {
    fprintf(stderr,
            "usage: %s <a_pin> <b_pin> <d_pin> <up_pin> <down_pin> <bias> "
            "<active_low> <debounce>\n",
            argv[0]);
    return 1;
  }
  ...
  if (get_lines(a_pin, b_pin, d_pin, up_pin, down_pin, argv[6], argv[7],
  ...
  // Sample every 10 ms
  struct itimerval signal_interval = {{0, 10000}, {0, 10000}};
