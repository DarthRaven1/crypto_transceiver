...
int main(int argc, char *argv[]) {
  ...
  short *speech_out =
      malloc(sizeof(short) * crypto_rx_max_speech_samples_per_frame(crypto_rx));
  short *demod_in =
      malloc(sizeof(short) * crypto_rx_max_modem_samples_per_frame(crypto_rx));
  ...
  while (read_input_file(demod_in, nin, fin) == nin) {
    ...
    if (reload_config != 0) {
      ...
      speech_out = realloc(
          speech_out,
          sizeof(short) * crypto_rx_max_speech_samples_per_frame(crypto_rx));
      demod_in = realloc(demod_in,
                         sizeof(short) *
                             crypto_rx_max_modem_samples_per_frame(crypto_rx));
    }
