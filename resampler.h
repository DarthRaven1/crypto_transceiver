...
inline size_t get_nom_resampled_frames(size_t src_frames, uint src_sample_rate,
...
inline size_t get_max_resampled_frames(size_t src_frames, uint src_sample_rate,
                                       uint dst_sample_rate) {
  const uint64_t src_frames_64 = src_frames;
  const uint64_t src_sample_rate_64 = src_sample_rate;
  const uint64_t dst_sample_rate_64 = dst_sample_rate;
...
static size_t resample_complete_buffer(int converter_type, int channels,
                                       const float *src_data, size_t src_frames,
                                       uint src_sample_rate, float *dst_data,
...
public:
  ...
  template <class Iterator> void enqueue(Iterator begin, Iterator end) {
    if (begin == end) {
      return;
  ...
  void enqueue(const short *data, size_t count) {
    if (count == 0) {
      ...
      src_short_to_float_array(data, m_resampled_data.data() + prev_size,
                               count);
    } else {
      const size_t prev_size = m_data_to_resample.size();
      m_data_to_resample.resize(prev_size + count);
      src_short_to_float_array(data, m_data_to_resample.data() + prev_size,
                               count);
  ...
  size_t available_elems() const { return m_resampled_data.size(); }
...
private:
  void do_resample(size_t max_elems_to_flush = 0) {
    ...
    m_data_to_resample.erase(m_data_to_resample.begin(),
                             m_data_to_resample.begin() +
                                 resample_parms.input_frames_used);
  }
