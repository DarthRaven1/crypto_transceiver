...
static void handle_sigptt(int sig) { sig_ptt_val = !sig_ptt_val; }
...
int process(jack_nframes_t nframes, void *arg) {
  ...
  if (transmitting_cur) {
    ...
    const uint modem_resampled_frames = get_nom_resampled_frames(
        n_nom_modem_samples, modem_sample_rate, jack_sample_rate);
...
static void activate_client() {
  ...
  if (period == 0) {
    ...
    snprintf(buffer, sizeof(buffer),
             ...
             period, modem_samples_per_frame, modem_sample_rate);
  } else {
  ...
  if (jack_connect(client, capture_port_name, jack_port_name(voice_port)) !=
      0) {
    fprintf(stderr, "Could not connect modem port");
    exit(1);
  }

  const char *playback_port_regex = *cfg->jack_modem_out_port
                                        ? cfg->jack_modem_out_port
                                        : "system:playback_*";
...
static void initialize_crypto() {
  ...
  const size_t speech_frames = get_nom_resampled_frames(
      crypto_tx->speech_samples_per_frame(), crypto_tx->speech_sample_rate(),
      jack_get_sample_rate(client));
  const size_t modem_frames = get_nom_resampled_frames(
      crypto_tx->modem_samples_per_frame(), crypto_tx->modem_sample_rate(),
      jack_get_sample_rate(client));
...
static void initialize_ptt() {
  ...
  if (cfg->ptt_enabled) {
    ...
    if (ptt_out_line != nullptr) {
      const int flags = cfg->ptt_output_bias | cfg->ptt_output_drive |
...
int main(int argc, char *argv[]) {
  ...
  */
  ...
  /* create two ports */
  voice_port = jack_port_register(client, "voice_in", JACK_DEFAULT_AUDIO_TYPE,
                                  JackPortIsInput, 0);

  modem_port = jack_port_register(client, "modem_out", JACK_DEFAULT_AUDIO_TYPE,
                                  JackPortIsOutput, 0);
