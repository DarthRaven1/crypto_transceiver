...
bool read_wav_file(const char *filepath, jack_nframes_t jack_sample_rate,
  ...
  while ((readcount = sf_readf_float(
              infile, (buffer.data() + buffer.size()) - block_len,
              block_len)) == block_len) {
  ...
  if (sfinfo.samplerate != jack_sample_rate) {
    const size_t max_resample_frames = get_max_resampled_frames(
        buffer.size(), sfinfo.samplerate, jack_sample_rate);
    audio_buffer_t resample_buffer(max_resample_frames);

    const size_t resample_frames = resample_complete_buffer(
        SRC_SINC_FASTEST, 1, buffer.data(), buffer.size(), sfinfo.samplerate,
        resample_buffer.data(), max_resample_frames, jack_sample_rate);
...
bool connect_input_ports(jack_client_t *client, jack_port_t *output_port,
  ...
  const char **playback_ports =
      jack_get_ports(client, input_port_regex, NULL, JackPortIsInput);
  if (playback_ports != NULL && *playback_ports != NULL) {
    ...
    for (size_t i = 0; playback_ports[i] != NULL; ++i) {
      if (jack_connect(client, out_port_name, playback_ports[i]) != 0) {
        fprintf(stderr, "Could not connect %s port to %s port\n", out_port_name,
                playback_ports[i]);
