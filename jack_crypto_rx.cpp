...
static void handle_sighup(int sig) { reload_config = 1; }

static void handle_sigusr1(int sig) { read_wav = 1; }
...
int process(jack_nframes_t nframes, void *arg) {
  ...
  const uint modem_resampled_frames =
      get_nom_resampled_frames(crypto_rx->modem_samples_per_frame(),
                               modem_sample_rate, jack_sample_rate);
  ...
  const uint voice_resampled_frames = get_nom_resampled_frames(
      n_nom_speech_samples, voice_sample_rate, jack_sample_rate);
  const size_t to_deque =
      std::min(output_resampler->available_elems(), (size_t)nframes);
  ...
  if (play_notification_sound) {
    ...
    if (crypto_stat == CRYPTO_STATUS_ENCRYPTED) {
      ...
      notification_buffer.insert(notification_buffer.cend(),
                                 plain_startup.cbegin(), plain_startup.cend());
    }
  }

  if (play_wave_sound) {
    notification_buffer.insert(notification_buffer.cend(), wave_sound.cbegin(),
                               wave_sound.cend());
  }

  jack_default_audio_sample_t *const notification_frames =
      (jack_default_audio_sample_t *)jack_port_get_buffer(notification_port,
                                                          nframes);
  if (notification_buffer.empty() == false) {
    const jack_nframes_t n_notification_frames = std::min(
        nframes, static_cast<jack_nframes_t>(notification_buffer.size()));
    const auto notification_end =
        notification_buffer.cbegin() + n_notification_frames;
    std::copy(notification_buffer.cbegin(), notification_end,
              notification_frames);
...
static void activate_client() {
  ...
  const uint speech_period = get_nom_resampled_frames(
      speech_samples_per_frame, speech_sample_rate, jack_sample_rate);
  ...
  if (period == 0) {
    ...
    snprintf(buffer, sizeof(buffer),
             ...
             period, speech_samples_per_frame, speech_sample_rate);
  } else {
    snprintf(buffer, sizeof(buffer), "Buffer size: %u from config file",
             period);
  }
  ...
  if (jack_connect(client, capture_port_name, jack_port_name(modem_port)) !=
      0) {
    fprintf(stderr, "Could not connect modem port");
    exit(1);
  }

  const char *voice_playback_port_regex = *cfg->jack_voice_out_port
                                              ? cfg->jack_voice_out_port
                                              : "system:playback_*";
  if (!connect_input_ports(voice_port, voice_playback_port_regex)) {
    exit(1);
  }

  const char *notify_playback_port_regex = *cfg->jack_notify_out_port
                                               ? cfg->jack_notify_out_port
                                               : "system:playback_*";
...
static void initialize_crypto() {
  ...
  const size_t speech_frames =
      get_max_resampled_frames(crypto_rx->max_speech_samples_per_frame(),
                               speech_sample_rate, jack_sample_rate);
...
int main(int argc, char *argv[]) {
  ...
  if (argc > 2) {
    server_name = argv[1];
    options =
        (jack_options_t)(JackNullOption | JackServerName | JackNoStartServer);
  ...
  */
  ...
  /* create two ports */
  voice_port = jack_port_register(client, "voice_out", JACK_DEFAULT_AUDIO_TYPE,
                                  JackPortIsOutput, 0);

  modem_port = jack_port_register(client, "modem_in", JACK_DEFAULT_AUDIO_TYPE,
                                  JackPortIsInput, 0);

  notification_port = jack_port_register(
      client, "notification_out", JACK_DEFAULT_AUDIO_TYPE, JackPortIsOutput, 0);
