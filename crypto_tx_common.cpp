...
struct crypto_tx_common::tx_parms {
  tx_parms() { memset(&logger, 0, sizeof(logger)); }
  ~tx_parms() {
    if (cur != nullptr)
      free(cur);
    if (freedv != nullptr)
...
crypto_tx_common::crypto_tx_common(const char *name, const char *config_file)
  ...
  m_parms->logger =
      create_logger(config_file_name.c_str(), m_parms->cur->log_level);

  if (m_parms->cur->freedv_enabled) {
    m_parms->freedv = freedv_open(m_parms->cur->freedv_mode);
    if (m_parms->freedv == NULL) {
      log_message(m_parms->logger, LOG_ERROR,
                  "Could not initialize voice modulator");
    }
  }
  ...
  if (m_parms->freedv != nullptr) {
    ...
    if (getrandom(iv, sizeof(iv), 0) != sizeof(iv)) {
      log_message(m_parms->logger, LOG_WARN,
    ...
    if (str_has_value(m_parms->cur->key_file) &&
      ...
      log_message(m_parms->logger, LOG_WARN,
                  ...
                  (int)key_bytes_read, (int)FREEDV_MASTER_KEY_LENGTH);
    }
...
void crypto_tx_common::force_rekey_next_frame() { m_parms->force_rekey = true; }
...
size_t crypto_tx_common::transmit(short *mod_out, const short *speech_in) {
  ...
  const int speech_frames_per_second =
      speech_samples_per_second / n_speech_samples;

  if (using_freedv() && str_has_value(m_parms->cur->key_file) &&
    ...
    const int rekey_frames =
        speech_frames_per_second * m_parms->cur->rekey_period;
    if (rekey_frames > 0 && (m_parms->frames_since_rekey % rekey_frames) == 0) {
      log_message(m_parms->logger, LOG_INFO,
...
HCRYPTO_TX *crypto_tx_create(const char *name, const char *config_file_path) {
  try {
    return reinterpret_cast<HCRYPTO_TX *>(
        new crypto_tx_common(name, config_file_path));
  } catch (...) {
    return NULL;
  }
}
...
int crypto_tx_transmit(HCRYPTO_TX *hnd, short *mod_out,
                       const short *speech_in) {
  try {
    return reinterpret_cast<crypto_tx_common *>(hnd)->transmit(mod_out,
                                                               speech_in);
  } catch (...) {
    return -1;
  }
