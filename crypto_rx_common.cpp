...
struct crypto_rx_common::rx_parms {
  rx_parms(const char *cfg) : config_file(cfg) {
  ...
  if (m_parms->cur->freedv_enabled != 0) {
    ...
    if (m_parms->freedv == NULL) {
      log_message(m_parms->logger, LOG_ERROR,
                  "Could not initialize voice demodulator");
    }
  }
  ...
  if (using_freedv()) {
    ...
    if (m_parms->modem_has_signal == true ||
        m_parms->modem_flush_frames <=
            m_parms->cur->modem_num_quiet_flush_frames) {
      nout =
          freedv_rx(m_parms->freedv, speech_out, const_cast<short *>(demod_in));
      ...
      log_message(m_parms->logger, LOG_DEBUG,
                  "nout: %u, SNR est.: %f, modem RMS: %d", (uint)nout, snr_est,
                  (int)modem_rms);
    }
...
HCRYPTO_RX *crypto_rx_create(const char *name, const char *config_file_path) {
  try {
    return reinterpret_cast<HCRYPTO_RX *>(
        new crypto_rx_common(name, config_file_path));
  } catch (...) {
    return NULL;
  }
}
...
int crypto_rx_max_speech_samples_per_frame(HCRYPTO_RX *hnd) {
  return reinterpret_cast<crypto_rx_common *>(hnd)
      ->max_speech_samples_per_frame();
}

int crypto_rx_max_modem_samples_per_frame(HCRYPTO_RX *hnd) {
  return reinterpret_cast<crypto_rx_common *>(hnd)
      ->max_modem_samples_per_frame();
}
...
int crypto_rx_receive(HCRYPTO_RX *hnd, short *speech_out,
                      const short *demod_in) {
  return reinterpret_cast<crypto_rx_common *>(hnd)->receive(speech_out,
                                                            demod_in);
}
