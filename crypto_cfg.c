...
static int ini_callback(const mTCHAR *Section, const mTCHAR *Key,
  ...
  if (strcasecmp(Section, "Crypto") == 0) {
    ...
    if (strcasecmp(Key, "Mode") == 0) {
      if (!strcasecmp(Value, "1600"))
        cfg->freedv_mode = FREEDV_MODE_1600;
      if (!strcasecmp(Value, "700C"))
        cfg->freedv_mode = FREEDV_MODE_700C;
      if (!strcasecmp(Value, "700D"))
        cfg->freedv_mode = FREEDV_MODE_700D;
      if (!strcasecmp(Value, "700E"))
        cfg->freedv_mode = FREEDV_MODE_700E;
      if (!strcasecmp(Value, "2400A"))
        cfg->freedv_mode = FREEDV_MODE_2400A;
      if (!strcasecmp(Value, "2400B"))
        cfg->freedv_mode = FREEDV_MODE_2400B;
      if (!strcasecmp(Value, "800XA"))
        cfg->freedv_mode = FREEDV_MODE_800XA;
